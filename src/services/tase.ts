...
export function isTaseOpen(): boolean {
  const now = new Date();
...
export async function fetchTaseQuote(ticker: string): Promise<Partial<StockQuote> | null> {
  ...
  try {
    ...
    if (!data?.securities?.length) return null;
